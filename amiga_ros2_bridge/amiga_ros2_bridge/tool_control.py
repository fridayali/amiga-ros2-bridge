...
TOOL_CONTROL_CFG = Path(__file__).resolve().parent / "include" / "tool_control_config.json"


async def send_hbridge_command(direction: str):
    """
    H-Bridge motor komutu gönderir (forward, reverse, stop).
    PATH sabittir, parametre istemez.
    """

    # Config'i JSON dosyasından oku
    config: EventServiceConfig = proto_from_json_file(TOOL_CONTROL_CFG, EventServiceConfig())
    client: EventClient = EventClient(config)

    # Komut struct'ı
    commands: ActuatorCommands = ActuatorCommands()
    print("reverse")
    # direction -> enum dönüştür
    if direction == "forward":
        commands.hbridges.append(HBridgeCommand(id=int(0), command=HBridgeCommandType.HBRIDGE_FORWARD))
    elif direction == "reverse":
        print("reverse")
        commands.hbridges.append(HBridgeCommand(id=int(0), command=HBridgeCommandType.HBRIDGE_REVERSE))
    elif direction == "stop":
        commands.hbridges.append(HBridgeCommand(id=int(0), command=HBridgeCommandType.HBRIDGE_STOPPED))
    else:
        raise ValueError("direction must be: forward / reverse / stop")

    # Komutu oluşturup ekle

    # Robot'a gönder
    await client.request_reply("/control_tools", commands, decode=True)

    print(f"[OK] Motor -> {direction}")
async def control_tools() -> None:
    ...
    while True:
        ...
        commands.hbridges.append(HBridgeCommand(id=int(0), command=HBridgeCommandType.HBRIDGE_FORWARD))
        # commands.hbridges.append(HBridgeCommand(id=int(0), command=HBridgeCommandType.HBRIDGE_FORWARD))
        # commands.hbridges.append(HBridgeCommand(id=int(1), command=HBridgeCommandType.HBRIDGE_FORWARD))
        # commands.hbridges.append(HBridgeCommand(id=int(2), command=HBridgeCommandType.HBRIDGE_FORWARD))
        # commands.hbridges.append(HBridgeCommand(id=int(3), command=HBridgeCommandType.HBRIDGE_FORWARD))
...
async def run(service_config_path: Path, keyboard_listener: KeyboardListener):
    ...
    tasks: list[asyncio.Task] = [
        asyncio.create_task(control_tools()),
...
if __name__ == "__main__":
    ...
    try:
        asyncio.run(run(Path("/home/cuma_karaaslan/farm-ng-amiga/py/examples/tool_control/service_config.json"), keyboard_listener))
